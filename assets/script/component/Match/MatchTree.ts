...
export default class MatchTree extends BaseUI {
    ...
    initData() {
            ...
            cc.tween(node)
                .repeat(data.stages[i].length,
                    cc.tween()
                        .delay(.1 * (idx + 1))
                        .call(() => {
                            UIMgr.OpenUI("component/Match/MatchTreeMode",
                                {
                                    parent: node,
        ...
        cc.tween(node)
            .delay(.1 * (idx + 1))
            .call(() => {
                UIMgr.OpenUI("component/Match/MatchTreeChapMode",
                    {
                        parent: node,
                        position: cc.v3(cc.winSize.width / 2),
                        name: "group_0",
        ...
        this.setActive("tip/award/wcoin", false)
        this.setActive("tip/award/lottery", false)
        this.setActive("tip/award/diamond", false)
        let k = 0
        for (let i in awards) {
            if (awards[i].id === Constants.ITEM_INDEX.WCOIN) {
                this.setActive("tip/award/wcoin", true)
                this.setLabelValue("tip/award/wcoin/num", Helper.FormatNumWY(awards[i].num))
            } else if (awards[i].id === Constants.ITEM_INDEX.LOTTERY) {
                this.setActive("tip/award/lottery", true)
                this.setLabelValue("tip/award/lottery/num", Helper.FormatNumWY(awards[i].num))
                this.setActive("tip/award/lottery/add", k > 0)
            } else if (awards[i].id === Constants.ITEM_INDEX.DIAMOND) {
                this.setActive("tip/award/diamond", true)
                this.setLabelValue("tip/award/diamond/num", awards[i].num >= 10000 ? Helper.FormatNumWY(awards[i].num) : Helper.FormatNumPrice(awards[i].num / 100))
                this.setActive("tip/award/diamond/add", k > 0)
            }
            k++
        }
    ...
    updateRound() {
        ...
        this.setActive("round/award/wcoin", false)
        this.setActive("round/award/lottery", false)
        this.setActive("round/award/diamond", false)
        // if (data && data.roundAwards) {
            ...
            for (let i in items) {
                if (items[i].id === Constants.ITEM_INDEX.WCOIN) {
                    this.setActive("round/award/wcoin", true)
                    this.setLabelValue("round/award/wcoin/num", Helper.FormatNumWY(items[i].num))
                } else if (items[i].id === Constants.ITEM_INDEX.LOTTERY) {
                    this.setActive("round/award/lottery", true)
                    this.setLabelValue("round/award/lottery/num", Helper.FormatNumWY(items[i].num))
                    this.setActive("round/award/lottery/add", idx > 0)
                } else if (items[i].id === Constants.ITEM_INDEX.DIAMOND) {
                    this.setActive("round/award/diamond", true)
                    this.setLabelValue("round/award/diamond/num", items[i].num >= 10000 ? Helper.FormatNumWY(items[i].num) : Helper.FormatNumPrice(items[i].num / 100))
                    this.setActive("round/award/diamond/add", idx > 0)
                }
                idx++
            }
        // }
    ...
    onPressDetail() {
        ...
        Helper.OpenPageUI("component/Match/MatchDetailEntry", "", null, { match: matchs[data.matchId] }, () => this.close())
    }
