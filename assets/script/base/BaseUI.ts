...
export default class BaseUI extends cc.Component {
    ...
    getNode(target: string | cc.Node, parent?: cc.Node) {
        ...
        if (typeof target === "string") {
            if (parent && parent instanceof cc.Node) {
                node = parent["_n" + target] || cc.find(target, parent)
                parent["_n" + target] = node
            } else {
                node = this.node["_n" + target] || cc.find(target, this.node)
                this.node["_n" + target] = node
            }
            // node = parent && parent instanceof cc.Node ? cc.find(target, parent) : cc.find(target, this.node)
        ...
        if (!node) {
            // if (typeof target === "string")
            cc.warn("target = " + target + " is not found in" + ((parent instanceof cc.Node) ? " parent" : " this.node"))
            // else
            // cc.warn("target type is not cc.Node")
    ...
    setSpriteFrame(name: string | cc.Node, parent?: cc.Node | cc.SpriteFrame | string, sprite: cc.SpriteFrame | string | boolean = null, isFixSize: boolean = null) {
        ...
        let updateFrame = (src) => {
            ...
            if (isFixSize) {
                ...
                // let scale = Math.max(size.width / newSize.width, size.height / newSize.height)
                // newSize.width *= scale
        ...
        if (sprite === null || sprite === "https://default") {
            ...
            if (-1 !== sprite.indexOf("http://") || -1 !== sprite.indexOf("https://")) {
                ...
                if (src) {
                    console.log("jin---tupian src: ", src)
                    updateFrame(src)
                } else {
                    this._pushRemoteQueue(sprite, updateFrame)
                }
                // cc.assetManager.loadRemote(sprite, cc.SpriteFrame, (err, res: cc.SpriteFrame) => {
                //     if (err) {
                ...
                let bundle = DataMgr.Bundle
                if (bundle) {
                    bundle.load(sprite, cc.SpriteFrame, (err, res: cc.SpriteFrame) => {
                        if (err) {
                            cc.warn("BaseUI.setSpriteFrame sprite " + sprite + " err: " + err)
                            return
                        }
    ...
    setLanguageId(name: string | cc.Node, parent: cc.Node | string, languageID?: string | any, param?: any) {
        if (!param && typeof languageID !== "string") {
            param = languageID
            languageID = undefined
        }

        if (typeof languageID !== "string" && typeof parent === "string") {
            languageID = parent
            parent = null
        }
        ...
        let node = this.getNode(name, parent instanceof cc.Node ? parent : null)
        if (node) {
            let local = node.getComponent(LocalSetting)
            if (!local) {
                local = node.addComponent(LocalSetting)
            }
