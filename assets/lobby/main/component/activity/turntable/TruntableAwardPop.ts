...
@ccclass
export default class TruntableAwardPop extends BaseUI {
    @property(cc.Layout)
    ...
    onOpen() {
        console.log("TruntableAwardPop onOpen", this.param)
        this.initEvent()
        this.initData()
        this.initButton()
        // this.updateData()
    ...
    initButton() {
        ...
        this.setButtonClick("node/btnDouble", () => {
            UIMgr.OpenUI("lobby", "component/activity/poker/Poker", { single: true, param: { dataInfo: this.param.dataInfo, awardInfo: this.param.awards } }, () => this.close())
        })
    }
    ...
    initData() {
        ...
        if (1 === this.param.dataInfo.param.poker_crap) {
            this.setNodePositionX("node/btnOpen", this.node, -150)
            this.setActive("node/btnDouble", this.node, true)
        }
    ...
    showAni() {
        ...
        ActivitySrv.GetDelayReward(param, () => {
            for (let i in this.param.awards) {
                let a = this.param.awards[i]
                if (a.item_id === Constants.ITEM_INDEX.LOTTERY) {
                    if (!this.param.noShowAni) {
                        User.Lottery -= Number(a.item_num)
                    }
